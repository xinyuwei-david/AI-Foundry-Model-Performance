...
# 如果你环境无法正常 import 这几个 SDK，请删除/注释掉  
try:  
...
def check_gpu_quota(resource_group, workspace_name):  
    ...
    SUPPORTED_AML_REGIONS = {  
        ...
        "southeastasia", "westeurope", "southcentralus", "canadaeast",  
        "swedencentral", "ukwest", "australiasoutheast", "qatarcentral", "southindia",  
        "polandcentral", "switzerlandwest", "italynorth", "spaincentral", "israelcentral"  
    }  
...
###############################################################################  
# CLI-based helpers for scoring_uri & keys  
###############################################################################  
def fetch_online_endpoint_info_cli(endpoint_name, resource_group, workspace_name, subscription_id):  
    """Use CLI commands to fetch scoring_uri & keys from an Online Endpoint."""  
    # Fetch scoring_uri  
    cmd_show = [  
        "az.cmd", "ml", "online-endpoint", "show",  
        "--name", endpoint_name,  
        "--resource-group", resource_group,  
        "--workspace-name", workspace_name,  
    ...
    result_show = subprocess.run(cmd_show, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)  
    scoring_uri = result_show.stdout.strip()  
  
    # Fetch Keys (primaryKey & secondaryKey)  
    cmd_keys = [  
        "az.cmd", "ml", "online-endpoint", "get-credentials",  
        "--name", endpoint_name,  
        "--resource-group", resource_group,  
        "--workspace-name", workspace_name,  
        "--subscription", subscription_id,  
    ...
    result_keys = subprocess.run(cmd_keys, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)  
    keys_json = json.loads(result_keys.stdout)  
    primary_key = keys_json.get("primaryKey")  
    secondary_key = keys_json.get("secondaryKey")  
  
    return scoring_uri, primary_key, secondary_key  
  
###############################################################################  
# Model deployment  
###############################################################################  
def deploy_model(subscription_id, resource_group, workspace_name, model_name, model_version, instance_type, instance_count):  
    """Deploy the model using Python SDK, but fetch the final scoring_uri & keys from CLI."""  
    ...
    # Configure 100% traffic to the new deployment  
    endpoint.traffic = {deployment_name: 100}  
    ml_client.online_endpoints.begin_create_or_update(endpoint).result()  
  
    # Use CLI commands to fetch scoring_uri & keys  
    logger.info("Fetching endpoint scoring URI & keys via CLI ...")  
    scoring_uri, primary_key, secondary_key = fetch_online_endpoint_info_cli(  
        endpoint_name, resource_group, workspace_name, subscription_id  
    )  
    ...
    # Print example usage code snippet  
...
print(response.json())'''  
    logger.info("You can test the deployment with code:\n" + example_code)  
...
def main():  
    ...
    subscription_id = prompt_or_default(  
        "Subscription ID: ", "08f95cfd-64fe-4187-99bb-7b3e661c4cde", IS_PRODUCTION  
    )  
    resource_group = prompt_or_default(  
        "Resource Group: ", "A100VM_group", IS_PRODUCTION  
    )  
    workspace_name = prompt_or_default(  
        "Workspace Name: ", "david-workspace-westeurope", IS_PRODUCTION  
    )  
    ...
    partial_str = prompt_or_default(  
        "Enter the model name to search (e.g., 'Phi-4'): ",  
        "Phi-4",  
        IS_PRODUCTION  
    )  
    print("\n========== Matching Models ==========")  
    ...
    for sku_name, gpu_count, vram, cpu_cores in sku_table:  
        print(f"{sku_name:<35} {gpu_count:<10} {vram:<20} {cpu_cores:<10}")  
    print()  
    ...
    # 7) Deploy the model (using Python SDK + final CLI queries)  
